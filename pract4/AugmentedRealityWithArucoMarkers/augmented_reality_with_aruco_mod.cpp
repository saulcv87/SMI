...
"{iOverlay y    | <none> | imagen sobrepuesta(overlay)}"
"{vOverlay o    | <none>  | video sobrepuesto(overlay)}"
;
using namespace cv;
...
int main(int argc, char** argv)
{
    ...
    Mat im_src;
    VideoCapture capOverlay;
    bool useVideoOverlay = false;
    if (parser.has("iOverlay")) {
        string overlayPath = parser.get<String>("iOverlay");
        im_src = imread(overlayPath);
        if (im_src.empty()) {
           cerr << "No se pudo cargar la imagen de overlay desde: " << overlayPath << endl;
           return -1;
        }
    
    } else if (parser.has("vOverlay")) {
        string overlayVideo = parser.get<String>("vOverlay");
        capOverlay.open(overlayVideo);
        if (!capOverlay.isOpened()) {
           cerr << "No se pudo cargar el video de overlay desde: " << overlayVideo << endl;
           return -1;
        }
        useVideoOverlay = true;
    } else {
       cerr << "Debe especificar una imagen (--iOverlay) o video (--vOverlay) para sobreponer." << endl;
       return -1;
    }
    ...
    while (waitKey(1) < 0)
    {
        ...
        try {
            ...
            imshow("DEBUG - Cámara", frame); // Mostrar imagen tal cual llega

            vector<int> markerIds;
            
            // Obtener el frame actual del overlay
             Mat overlayFrame;
            static Mat lastValidOverlay;

            if (useVideoOverlay) {
                 capOverlay >> overlayFrame;
                 if (!overlayFrame.empty()) {
                      im_src = overlayFrame.clone();  // Usar el frame actual
                       lastValidOverlay = overlayFrame.clone();  // Guardar como último válido
                 } else if (!lastValidOverlay.empty()) {
                         im_src = lastValidOverlay.clone();  // Mantener último frame válido si terminó el video
                 } else {
                        cerr << "Error: No hay frame válido del video overlay." << endl;
                        break;
                 }
            }
            ...
            // Verificar si todos los marcadores requeridos están presentes
            vector<int> requiredMarkers = {25, 33, 30, 23};
            bool allFound = true;
            for (int id : requiredMarkers) {
            ...
            if (!allFound) {
               cerr << "No se encontraron todos los marcadores requeridos (25, 33, 30, 23) en el frame." << endl;
       ...
       //         capOverlay >> overlayFrame;
       //         if (overlayFrame.empty()) {
       //            // Reiniciar el video overlay si llega al final
       //             capOverlay.set(CAP_PROP_POS_FRAMES, 0);
       //             capOverlay >> overlayFrame;
       //         }
       //         if (overlayFrame.empty()) {
       //             cerr << "Error al leer frame del video overlay." << endl;
       //             break;
       //         }
       //           // Redimensionar para que coincida con el tamaño del overlay esperado
       //         im_src = overlayFrame;
       //     }
